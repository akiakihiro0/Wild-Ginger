# Wild-Ginger


# Wild Ginger — Restaurant Website Replica

A modern front-end replica of the official [Wild Ginger restaurant website](https://www.wildginger.net/), recreated as a personal web development project and deployed with GitHub Pages.

This project focuses on responsive UI design, modern layout structure, visual hierarchy, and recreating the premium dining experience of the original website using front-end technologies.

## Live Demo

[View Live Website](https://akiakihiro0.github.io/Wild-Ginger/)

## About the Project

This is a non-commercial educational project inspired by the design and branding of the original [Wild Ginger restaurant website](https://www.wildginger.net/). The goal was to practice:

* Advanced front-end layout recreation
* Responsive restaurant website design
* Modern typography and spacing
* Hero sections and visual storytelling
* Navigation structure and CTA placement
* Restaurant-style UI/UX patterns

The original restaurant was established in 1989 and is known for Southeast Asian-inspired fine dining in Seattle. ([Wild Ginger][1])

## Features

* Fully responsive design
* Modern restaurant landing page
* Hero image sections
* Reservation and menu sections
* Smooth navigation layout
* Private events and wine program sections
* Elegant typography and visual hierarchy
* GitHub Pages deployment

## Tech Stack

* HTML5
* CSS3
* JavaScript
* GitHub Pages

## Screenshots

You can add screenshots here later using:

#Homepage
![Homepage Screenshot](https://github.com/akiakihiro0/Wild-Ginger/blob/604ac373745dcb3b2e422b7c68f423737ebc33f7/home.png)

#Paralax
![Paralax Screenshot](https://github.com/akiakihiro0/Wild-Ginger/blob/604ac373745dcb3b2e422b7c68f423737ebc33f7/paralax.png)

#Grid
![Grid Screenshot](https://github.com/akiakihiro0/Wild-Ginger/blob/604ac373745dcb3b2e422b7c68f423737ebc33f7/grid.png)

#Form
![Form Screenshot](https://github.com/akiakihiro0/Wild-Ginger/blob/604ac373745dcb3b2e422b7c68f423737ebc33f7/form.png)

## Installation

Clone the repository:

```bash
git clone https://github.com/akiakihiro0/Wild-Ginger.git
```

Open the project folder:

```bash
cd Wild-Ginger
```

Run locally by opening `index.html` in your browser.

## Deployment

This project is deployed using:

[GitHub Pages](https://pages.github.com/)

## Disclaimer

This project is created strictly for educational and portfolio purposes.

All branding, restaurant names, images, and design inspiration belong to the original [Wild Ginger restaurant](https://www.wildginger.net/) and their respective owners.

No commercial use or copyright infringement is intended.

## Future Improvements

* Improved mobile responsiveness
* Better accessibility support
* Image optimization
* Dark mode support
* Animated transitions
* SEO enhancements
* Backend reservation system integration

## Author

Created by Sulakshyan Bolakhe

GitHub: [akiakihiro0 GitHub Profile](https://github.com/akiakihiro0)

[1]: https://www.wildginger.net/ "Wild Ginger"
